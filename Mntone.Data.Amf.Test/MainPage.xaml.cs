...
namespace Mntone.Data.Amf.Test
{
	public sealed partial class MainPage: Page
	{
		...
		private void Start()
		{
			...
			Output( "// Change position in std::map when parse.\n" );
			Output( "\n" );
			// ------------
		}
		...
		private void TestAmf0( string start, byte[] rawData, Func<IAmfValue, bool> readCheck )
		{
			...
			sb.Append( ", " );
			var createData = Amf0Sequencer.Sequenceify( r );
			sb.Append( TextOkOrNg( ArrayEquals( rawData, createData ) ) ); // Sequencer check
			Output( sb.ToString() + '\n' );
		}
